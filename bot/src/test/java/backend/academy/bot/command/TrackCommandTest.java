...
import backend.academy.bot.command.BaseCommandTest;
...
public class TrackCommandTest extends BaseCommandTest {
    ...
    void handleCorrectUrlShouldReturnSuccessResponse() {
        ...
        // Assert
        assertEquals("Введите теги через пробел для ссылки", sendMessage.getParameters().get("text"));
    }
    ...
    void handleIncorrectUrl() {
        ...
        doThrow(new InvalidInputFormatException("Use a valid URL as a parameter in the form like '/track <link>'"))
                .when(parserMessage).parseUrl(commandMessage, UserState.WAITING_URL);

        // Act
        SendMessage sendMessage = trackCommand.handle(update);

        // Assert
        assertEquals("Use a valid URL as a parameter in the form like '/track <link>'", sendMessage.getParameters().get("text"));
    }
    ...
    void handleDuplicateLink() {
        ...
        // Assert
        assertEquals("Такая ссылка уже добавлена, добавьте новую ссылку используя /track", sendMessage.getParameters().get("text"));
    }
    ...
    void handleInvalidTagsInput() {
        ...
        doThrow(new InvalidInputFormatException("Теги не могут быть пустыми"))
                .when(parserMessage).getAdditionalAttribute(invalidTagsMessage);
    ...
    void handleInvalidFiltersInput() {
        ...
        // Устанавливаем состояние пользователя в WAITING_FILTERS
        when(userStateManager.getUserState(chatId)).thenReturn(UserState.WAITING_FILTERS);
        ...
        // Assert
        assertEquals("Фильтры не могут быть пустыми", sendMessage.getParameters().get("text"));
    }
}
